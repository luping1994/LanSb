...
import com.suntrans.lanzhouwh.convert.Converts;
            ...
            new Thread() {      //不能在主线程中访问网络，所以要新建线程
                public void run() {   //新建线程连接服务器，不占用主线程
                    try {
                        ...
                        LogUtil.i("ip地址为:" + serverAddr.getHostAddress());
                        LogUtil.i("端口号:" + port);
                        ...
                        String toServer = "aa67" + SerialNumber;  //发送手机地址
...
//                        Log.i("Order", str);
        ...
        binder = new ibinder() {
            @Override
            public boolean sendOrder(final String order) {   //重写发送命令的方法
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            try {
                                ...
                                String toServer = "aa67" + SerialNumber;  //发送手机地址，发送后手机才能接收到服务器的数据
                                toServer = toServer.replace(" ", "");    //去掉空格
                                byte[] bt = null;
                                bt = Converts.HexString2Bytes(toServer);
                                String str = toServer + Converts.GetCRC(bt, 0, bt.length);   //添加校验码
                                ...
                                if (!client.isClosed()) {
                                    out.write(bt1);
                                    out.flush();
                                    System.out.println("发送命令"+toServer);
                                }
                            } catch (Exception ee) {
...
//            SerialNumber="0000";
        ...
        public void run() {
            ...
            try {
                ...
                while ((count = ins.read(content)) != -1) {     //读取数据 ，存放到缓存区content中
                    ...
                    s = s.replace(" ", ""); //去掉空格
                    String[] results = s.split("aa67");
                    for (String ss :
                            results) {
                        ss="aa67"+ss;
                        if (ss.length()==8||ss.length()==10||ss.length()==12){
                            byte[] tem = Converts.HexString2Bytes(ss);   //转换成byte数组
                            if (tem.length != 0) {
                                Intent intent = new Intent();
                                intent.setAction("com.suntrans.bejing.RECEIVE");
    ...
    public class ibinder extends Binder {
         ...
         */
        public synchronized boolean sendOrder(String order) {
            return true;
        }
    }
